...
//todo: verify this is fisher-yates shuffle
function shuffleSet(set) {
  var values = set.copy()
  for (var index = values.length; index > 0; index -= 1) {
    var randomIndex = randomBoundedInteger(0, index)
...
function setGenerator(values, replacement, shuffle, weights) {
  ...
  if (shuffle) {
    return function() { return shuffleSet(set) }
  } else if (replacement) {
    if (weights) 
...
Stochator.prototype.createGenerator = function(config) {
  ...
  switch (config.kind) {
    ...
    case "set":
      var values = config.values, replacement = config.replacement, shuffle = config.shuffle, weights = config.weights;
